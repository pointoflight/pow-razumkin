...
const STATUSES = [
  ...
  { value: 'in_progress', label: 'В работе', color: 'bg-orange-100 text-orange-800' },
...
export default function EducatorLeadsPage() {
  ...
  function fetchLeads() {
    ...
    setLoading(true);
    fetch(`/api/educators/my/leads?${params}`, { credentials: 'include' })
      .then((r) => r.json())
  ...
  const getStatusStyle = (s: string) => STATUSES.find((x) => x.value === s)?.color || 'bg-gray-100 text-gray-700';
  const getStatusLabel = (s: string) => STATUSES.find((x) => x.value === s)?.label || s;

  return (
    <div className="space-y-5">
      <h1 className="text-2xl font-bold text-gray-900">Заявки от родителей</h1>
      ...
      {loading ? (
        ...
        <div className="text-center py-20 bg-white border border-gray-200 rounded-xl">
          ...
          <p className="text-sm text-gray-400 mt-1">Родители смогут написать вам с вашей публичной страницы</p>
        </div>
      ) : (
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                ...
                <th className="text-left px-5 py-3 font-medium text-gray-600 hidden md:table-cell">Ребёнок</th>
          ...
          <div className="bg-white rounded-2xl p-6 w-full max-w-lg shadow-2xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-3 text-sm mb-6">
              <div>
                ...
                <a href={`tel:${selected.parentPhone}`} className="text-primary-600 font-medium">{selected.parentPhone}</a>
              </div>
              {selected.child && (
                <div>
