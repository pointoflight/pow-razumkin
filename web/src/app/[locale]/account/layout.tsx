...
export default async function AccountLayout({ children }: { children: React.ReactNode }) {
  ...
  const NAV = [
    { href: `/${locale}/account`, label: 'Обзор', icon: '👤' },
    { href: `/${locale}/account/children`, label: 'Мои дети', icon: '👧' },
    { href: `/${locale}/account/requests`, label: 'Мои заявки', icon: '📋' },
  ];
        ...
        <aside className="lg:w-56 flex-shrink-0">
          <nav className="bg-white border border-gray-200 rounded-xl overflow-hidden">
            <div className="px-4 py-4 bg-primary-600">
              <p className="text-white text-xs font-medium uppercase tracking-wide">Мой аккаунт</p>
            </div>
            {NAV.map((item) => (
              <Link
                key={item.href}
